...
func main() {
	...
	screener, err := hunter.New(hunter.Config{
		...
		ExactWorkers:                   int(envUint64("PHOENIX_AAVE_EXACT_WORKERS", 8)),
		SignalSink:                     sink,
	})
	if err != nil {
...
func healthPayload(health laneHealth, state hunter.State, atlasConnected bool) map[string]any {
	return map[string]any{
		...
		"aave_exact_actionable_queue":           state.ExactEligibleNowCount,
		"aave_exact_in_flight_queue":            state.ExactEvaluationsInFlight,
		"aave_exact_worker_queue_depth":         state.ExactWorkerQueueDepth,
		"aave_exact_blocked_queue": state.SchedulerBlockedCount + state.CooldownBlockedCount +
			state.RouteIneligibleCount + state.ProviderBlockedCount + state.AuthorityBlockedCount,
		...
		"aave_exact_coalesced_total":              state.Counts["exact_coalesced_total"],
		"aave_exact_stale_invalidated_total":      state.Counts["exact_stale_invalidated_total"],
		"aave_exact_duplicate_suppressed_total":   state.Counts["exact_duplicate_suppressed_total"],
