...
class WorkflowAndDeploymentContractTests(unittest.TestCase):
    ...
    def _forward_fix_bootstrap_validator(self) -> str:
        match = re.search(
            r"^\s*# FORWARD_FIX_BOOTSTRAP_VALIDATOR_BEGIN\n"
            r"(?P<source>.*?)"
            r"^\s*# FORWARD_FIX_BOOTSTRAP_VALIDATOR_END$",
            self.workflow,
    ...
    @staticmethod
    def _forward_fix_bootstrap_evidence(
        ...
        for index, name in enumerate(service_names, start=1):
            ...
            services.append(
                {
                    "configured_image": f"example.invalid/{name}@sha256:{digest}",
        ...
        atlas.update(
            {
                ...
                "mounts": [
                    {
                        "destination": "/var/lib/phoenix-atlas-observer",
                        "mode": "rw",
                        "source": "/opt/phoenix/evidence/atlas-live",
                        "type": "bind",
                    }
                ],
            }
        ...
        readiness = {
            ...
            "failures": [
                {
                    "code": "READINESS_SERVICE_UNHEALTHY",
                    "evidence": copy.deepcopy(atlas),
                }
            ],
            "schema": "phoenix.production-readiness.v1",
            "status": "failed",
        ...
        failed = {
            "active_release_pointer": active_release,
            "autonomous_armed": False,
            "candidate_pointer": failed_release,
            "contract_paused": True,
            ...
            "owner_transaction_hash": None,
            "release_sha": failed_release,
            "rollback_sha": active_release,
        }
        return raw, failed

    def _run_forward_fix_bootstrap(
        self,
        raw: dict[str, object],
        failed: dict[str, object],
        *,
        candidate_sha: str,
        rollback_sha: str = "1776be2f7e9d2921589e4b4eb0faf5fe057e38fb",
        cwd: Path = ROOT,
    ) -> tuple[subprocess.CompletedProcess[str], dict[str, object] | None]:
        raw["evidence"]["candidate_sha"] = candidate_sha
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            raw_path = root / "readiness.json"
            ...
            completed = subprocess.run(
                [
                    sys.executable,
                    "-c",
                    self._forward_fix_bootstrap_validator(),
                    candidate_sha,
                    rollback_sha,
                    str(raw_path),
                    str(failed_path),
    ...
    def test_forward_fix_bootstrap_accepts_only_exact_poisoned_state_incident(
        self,
    ) -> None:
        ...
        raw, failed = self._forward_fix_bootstrap_evidence()
        completed, result = self._run_forward_fix_bootstrap(
            raw, failed, candidate_sha=candidate
        )
        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertEqual(
            result["schema"],
            "phoenix.production-readiness-forward-fix-bootstrap.v1",
        )
        self.assertTrue(result["controls_fail_closed"])
        self.assertTrue(result["contract_paused"])
        self.assertTrue(result["zero_work"])
        self.assertTrue(result["owner_transaction_absent"])
        self.assertEqual(result["accepted_service"], "atlas-observer")

        cases = [
            (
                "wrong_active_release",
                lambda readiness, state: readiness["evidence"]["checks"][
                    "active_release"
                ].update(active_release="0" * 40),
            ),
            (
                "wrong_failed_release",
                lambda readiness, state: state.update(release_sha="0" * 40),
            ),
            (
                "wrong_rollback_release",
                lambda readiness, state: state.update(rollback_sha="0" * 40),
            ),
            (
                "non_rollback_failed",
                lambda readiness, state: state.update(current_phase="ROLLED_BACK"),
            ),
            (
                "owner_transaction",
                lambda readiness, state: state.update(owner_transaction_hash="0x01"),
            ),
            (
                "contract_unpaused",
                lambda readiness, state: readiness["evidence"]["checks"][
                    "contract"
                ].update(contract_paused=False),
            ),
            (
                "provider_failure",
                lambda readiness, state: readiness["evidence"].update(
                    failures=[
                        {
                            "code": "READINESS_RPC_PROVIDER_UNAVAILABLE",
                            "evidence": {},
                        }
                    ]
                ),
            ...
            (
                "second_unhealthy_service",
                lambda readiness, state: (
                    readiness["evidence"]["checks"]["services"][1].update(
                        health="unhealthy"
                    ),
                    readiness["evidence"].update(failure_count=2),
                    readiness["evidence"]["failures"].append(
                        {
                            "code": "READINESS_SERVICE_UNHEALTHY",
            ...
            (
                ...
                lambda readiness, state: (
                    readiness["evidence"]["checks"]["expected_services"].append(
                        "live-executor"
                    ),
                    readiness["evidence"]["checks"]["services"].append(
                        {
                            "configured_image": (
                                "example.invalid/live-executor@sha256:" + "e" * 64
                            ),
                            "container_id": "f" * 64,
                            "health": "healthy",
        ...
        for lane in ("aave_armed", "atlas_armed", "armed"):
            cases.append(
                (
                    f"lane_armed_{lane}",
                    lambda readiness, state, field=lane: readiness["evidence"][
                        "checks"
                    ]["controls"].update({field: True}),
                )
            )
        for field, value in (
            ("active_attempts", 1),
            ...
            ("submission_lock_free", False),
        ):
            cases.append(
                (
                    f"unresolved_work_{field}",
                    lambda readiness, state, key=field, actual=value: readiness[
                        "evidence"
                    ]["checks"]["controls"].update({key: actual}),
                )
            )
        for name, mutation in cases:
            with self.subTest(name=name):
                candidate_raw, candidate_failed = (
                    self._forward_fix_bootstrap_evidence()
                )
                mutation(candidate_raw, candidate_failed)
                rejected, artifact = self._run_forward_fix_bootstrap(
                    candidate_raw,
                    candidate_failed,
        ...
        with tempfile.TemporaryDirectory() as unrelated:
            unrelated_repo = Path(unrelated)
            subprocess.run(
                ["git", "init", "-q"], cwd=unrelated_repo, check=True
            )
            subprocess.run(
            ...
            (unrelated_repo / "unrelated").write_text("unrelated\n")
            subprocess.run(
                ["git", "add", "unrelated"], cwd=unrelated_repo, check=True
            )
            subprocess.run(
            ...
            self.assertNotEqual(rejected.returncode, 0)
            self.assertIn(
                "candidate_missing_forward_fix", rejected.stderr
            )
            self.assertIsNone(artifact)
