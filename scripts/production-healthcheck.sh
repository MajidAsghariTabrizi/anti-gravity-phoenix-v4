...
compose() {
  PHOENIX_ENV_FILE="$env_file" docker compose --env-file "$env_file" --env-file "$release_env" -f "$compose_file" "$@"
}
...
check shadow-mode compose exec -T phoenix-engine /bin/sh -c '[ "$PHOENIX_MODE" = SHADOW ] && [ "$LIVE_EXECUTION" = false ]'
