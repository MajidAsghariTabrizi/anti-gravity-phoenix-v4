...
contract MockAavePool is IAaveV3Pool {
        ...
        override
    {
        ...
        bool ok = IAaveFlashBorrower(receiverAddress).executeOperation(asset, amount, premium, receiverAddress, params);
        require(ok, "callback");
...
contract PhoenixExecutorTest {
    ...
    function opportunity(uint256 minProfit, uint256 deadline) internal view returns (PhoenixExecutor.Opportunity memory op) {
    ...
    function testFakeV3Callback() public {
        setUp();
        try executor.uniswapV3SwapCallback(1, 0, abi.encode(PhoenixExecutor.SwapCallbackData(address(usdc), address(weth), address(this)))) {
...
contract Attacker {
    function tryExecute(PhoenixExecutor executor, PhoenixExecutor.Opportunity memory op) external returns (bool) {
        try executor.executeOpportunity(op) {
            return true;
        } catch {
            return false;
