...
- Live eval runner (`tools/phoenix-harness-v3/src/eval/`): each (task, arm, run) is one child process booting the pinned checkout with a mounted driver row; arms get isolated `DSH_HOME` copies (settings/credentials/agent-presets, never sessions); provider TRANSPORT failures and stalled children retry per attempt (max 3); killed runs preserve worktree telemetry so gate statistics stay complete. Promotion gate evidence must come from `eval-live gates --final` over real runs — synthetic records can never pass.
