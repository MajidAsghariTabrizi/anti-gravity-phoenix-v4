...
#[test]
fn revenue_lane_activation_rejects_missing_acknowledgement_before_database_access() {
    let output = control_command("arm-revenue-lanes")
        .output()
        .expect("run revenue lane activation without acknowledgement");
    let error = stderr(&output);

    assert!(!output.status.success());
    assert!(output.stdout.is_empty());
    assert!(error.contains(
        "AUTONOMOUS_CONTROL_FAILED: required environment is missing: PHOENIX_REVENUE_LANES_ACK"
    ));
    assert!(!error.contains("POSTGRES_DSN"));
    assert!(!error.contains("SIGNER_PRIVATE_KEY"));
}
